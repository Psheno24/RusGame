...
function feedTagClass(type: CityFeedEvent["type"]): string {
  if (type.startsWith("work:")) return "city-feed-tag city-feed-tag--work";
  if (type.startsWith("travel:")) return "city-feed-tag city-feed-tag--travel";
  return "city-feed-tag city-feed-tag--shop";
}

function feedTagLabel(type: CityFeedEvent["type"]): string {
  if (type === "travel:arrive" || type === "travel:depart") return "Поездка";
...
export function CityActivityFeed({ cityName, events, nowMs = Date.now() }: Props) {
  ...
  return (
    <section className="city-feed" aria-label="Лента активности города">
      ...
      {sorted.length === 0 ? (
        <p className="city-feed-empty">Пока тихо — здесь появятся события игроков.</p>
      ) : (
