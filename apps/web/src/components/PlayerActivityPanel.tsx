...
export function PlayerActivityPanel({ open, onClose }: Props) {
  ...
  return (
    <div className="player-feed-overlay" role="presentation" onClick={onClose}>
      <div
        className="player-feed-panel"
        role="dialog"
        aria-label="Моя активность"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="player-feed-panel-header">
          <h2 className="player-feed-panel-title">Моя активность</h2>
          <button type="button" className="player-feed-close" onClick={onClose} aria-label="Закрыть">
            ×
          </button>
        </div>
        {loading ? (
          ...
          <p className="player-feed-empty">Пока нет записей — поездки, работа и покупки появятся здесь.</p>
        ) : (
